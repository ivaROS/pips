...
namespace pips
{
  ...
  namespace collision_testing
  {
    ...
    namespace robot_models
    {
      ...
      public:
        RobotModel(ros::NodeHandle nh, ros::NodeHandle pnh, tf2_utils::TransformManager tfm=tf2_utils::TransformManager(false), std::string name="robot_model");
      ...
      protected:
        ...
        virtual std::shared_ptr<geometry_models::GenericGeometryModel> getGeometry(const urdf::Cylinder& cylinder);

        std::shared_ptr<geometry_models::GenericGeometryModel> getGeometry(const urdf::Sphere& cylinder);
        
        virtual std::shared_ptr<geometry_models::GenericGeometryModel> getGeometry(const urdf::Box& box);
